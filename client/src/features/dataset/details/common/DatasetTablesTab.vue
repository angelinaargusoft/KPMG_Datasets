<template>
  <!-- search -->
  <div class="d-flex justify-end align-center ma-8 flex-wrap" style="gap: 12px">
    <v-text-field
  ...
  >
    <template #rows="{ items }">
      ...
      <tr v-for="row in items" :key="row.name"></tr>
    </template>
  </BaseTable>
</template>
...
// temp
const tablesList = computed(() => store.getters["datasetTables/items"] || []);

const loadingTables = computed(() => store.getters["datasetTables/loading"]);
